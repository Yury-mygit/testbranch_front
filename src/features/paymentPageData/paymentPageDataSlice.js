...
const sigPayBefore3DS = (paramsForPay=[]) => {
    ...
    if (paramsForPay.length ===0) return 'undefined'
...
export const paymentPageDataSlice = createSlice({
  ...
  reducers: {
    update: (state, action) => {
      ...
      let a = data.findIndex(value => value.id === item.id )
      let c = structuredClone(data)
      // console.log(c[a])
      c[a].data=newdata
    ...
    setChecked: (state, action)=>{
        ...
        let a = data.filter(i=>i.id!=item.id)
...
export default paymentPageDataSlice.reducer